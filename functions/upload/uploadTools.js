...
export function isExtValid(fileExt) {
  return ["jpeg", "jpg", "png", "gif", "webp", "ico", "svg"].includes(fileExt);
}
...
export async function moderateContent(env, url) {
  ...
  if (uploadModerate.channel === "nsfwjs" || uploadModerate.channel === "default") {
    ...
    const nsfwApiPath =
      uploadModerate.channel === "default" ? defaultApiPath : securityConfig.upload.moderate.nsfwApiPath;
...
export async function endUpload(context, fileId, metadata) {
  ...
  const normalizedFolder = (url.searchParams.get("uploadFolder") || "")
    .replace(/^\/+/, "")
    .replace(/\/{2,}/g, "/")
    .replace(/\/$/, "");
...
export function getUploadIp(request) {
  const ip =
    ...
    request.headers.get("requester") ||
    request.headers.get("true-client-ip") ||
    request.headers.get("client-ip") ||
    request.headers.get("x-remote-ip") ||
    request.headers.get("x-originating-ip") ||
    request.headers.get("fastly-client-ip") ||
    request.headers.get("akamai-origin-hop") ||
