...
async function uploadFileToTelegram(context, fullId, metadata, fileExt, fileName, fileType, returnLink) {
  ...
  const tgChannel = tgSettings.loadBalance.enabled
  ...
  // 所有文件都作为文件发送，避免被Telegram压缩，缩略图等问题
  ...
  try {
    ...
    // 将响应返回给客户端
    res = createResponse(JSON.stringify([{ src: `${returnLink}` }]), {
      status: 200,
      headers: {
        "Content-Type": "application/json",
      },
